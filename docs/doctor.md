...
| D06 | the embedded pricing table is fresher than `now − 90 days`        | none; reports                                                      |
| D07 | `CLAUDE_CONFIG_DIR` (when set) points at a writable directory     | none; reports                                                      |
| D08 | update-check cache (read-only) reports a newer release            | none; reports                                                      |
| D09 | render dry-run on an embedded fixture matches the stored snapshot | none; reports                                                      |
...
that table on a monthly schedule. D06 reports `warn` when the table
is older than 90 days so you can upgrade `@agentline/cli`. No fix.

### D07 — config-dir writability
...
so a stale pricing table surfaces in the build log instead of
silently shipping.
