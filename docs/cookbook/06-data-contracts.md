...
| `thinkingEffort`    | `effort`              | object  | `{ level }` — `low` / `medium` / `high` / `xhigh` / `max`                                                                                                                                                                                                                                                                                                                                                                       |
...
| `workspace`         | `workspace`           | object  | `{ current_dir, project_dir, added_dirs, repo }` — `project_dir` feeds the project-dir widget, `added_dirs` the added-dirs widget, `current_dir` is the `cwd` fallback, and the optional nested `repo: { host, owner, name }` feeds the `git-origin-repo` widget's host-first name and `owner/name` variant                                                                                                                     |
...
| `skills`            | `skills`              | array   | Host-session subagent skills the host has loaded for this session — the inbound stdin field, **distinct** from the agentline-shipped skill files the installer copies into the host's agents directory (`04 · State surfaces`, `15-documentation-set · Shipped skill files`)                                                                                                                                                    |
...
| Other host fields   |                       | …       | Preserved untouched. Host keys agentline intentionally ignores (e.g. `hook_event_name`) are allowlisted in `scripts/check-host-contract.mjs`                                                                                                                                                                                                                                                                                    |
