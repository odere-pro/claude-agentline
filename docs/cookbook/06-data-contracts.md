...
| `context_window`    | `context_window`      | object  | Current-prompt window snapshot: `{ current_usage: { input_tokens, cache_read_input_tokens, cache_creation_input_tokens }, context_window_size, used_percentage }`. The three `current_usage` components sum to used tokens; `cache_read_input_tokens + cache_creation_input_tokens` is exposed separately as the cached portion of the live prompt. With the window size and host-computed percentage they feed the context-window / context-percentage / tokens-cached widgets (`adaptContextWindow`, `src/core/stdin/index.ts`) |
