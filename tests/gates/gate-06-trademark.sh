...
allowlist='(Claude Code statusline|Claude Code'\''s statusline|Claude Code reads|Claude Code contract|Claude Code stdin|Claude Code settings|Claude Code'\''s settings|Claude Code state|Claude Code'\''s state|wired into Claude Code|with Claude Code|of Claude Code|to Claude Code|from Claude Code|for Claude Code|for-Claude%20Code|inside Claude Code|without leaving Claude Code|CLAUDE_(CONFIG_DIR|PROJECT_DIR)|honour Claude Code|by Claude Code|Claude Code session|Claude Code version|Claude Code run|Claude Code.*statusLine|Restart Claude Code|Claude Code REPL keyboard|Anthropic\b.{0,40}\bnpm|logo=anthropic|Claude project|Claude-project|send Claude a|Claude Code invokes|inside Claude Code sessions|Claude Code sends|Claude Code reports|Claude Code didn|Claude Code is authoritative|sees in Claude Code|older Claude Code|Claude Code'\''s user-facing|Claude Code'\''s pre-computed|Claude Code'\''s contract|Claude Code'\''s `context|Claude Code'\''s `display|Claude Code'\''s own UI|Claude Code prompt|Claude Code most recently|when Claude Code has|Claude Code transcript|Claude Code data|Claude Code has( ever)? run|Claude Code falls back|Claude Code reverts to event-driven|Claude Code refreshes on|[Mm]irrored into Claude|Claude Code CLI|Claude CLI|Claude-CLI|Claude doctor)'
