...
# back to rendering the raw id as-is, prefixed with its (session)
# family glyph. `AGENTLINE_GLYPHS=ascii` pins that glyph to `[s]`
# so the expected bytes are host-independent. The rest of the
# default config widgets hide (no transcript, no git repo, etc.).
...
actual="$(printf '%s' "${fixture_in}" | NO_COLOR=1 AGENTLINE_GLYPHS=ascii CLAUDE_CONFIG_DIR="${isolated_cfg}" "${bin}" 2>"${work_root}/render.err")" \
