...
- `tui/` — the Ink app shell (`app.ts`, `mount.ts`, `main/`, `footer.ts`, `glyphs/`, `persist/`, the `use/` hooks, the `editor/` input handlers).
- `picker/` — the widget/family/variant picker overlays.
- `preview/` — the live preview (fixture / discovery / mock sources) and the preview-live parity guard.
- `state/` — the reducer-style state machine (`state.ts`, `state-core.ts`, `state-mutations.ts`, `state-picker.ts`).
...
```
src/tui/
├── tui/      Ink app shell, root component (app.ts, mount.ts), main/, glyphs/, footer.ts,
│             persist/, use/ hooks, editor/ input handlers
├── state/    pure reducer (actions in → immutable state out; no Ink/React imports here)
├── picker/   widget / family / variant picker overlays
├── preview/  live preview — waterfall: cache → discovered → mock (TUI-only)
└── keys/     keymap registry (DEFAULT_KEY_BINDINGS) — own tsup output dist/keys.mjs

  ▲ reached ONLY via runtime URL import in src/cli/cli.ts:
    await import(new URL("./tui.mjs", import.meta.url).href)
    gate-19 forbids any static import of ink / react / src/tui/ from render-reachable files
```

Patterns: **Lazy import (cold-path isolation)** + **Capability flag for editor scopes** (`docs/cookbook/05-design-patterns.md`).
...
- The editor is loaded solely via the lazy runtime URL import `new URL("./tui.mjs", …)` in `src/cli/cli.ts` — a deliberate runtime string, not a static import, so the analyser never resolves it against the render tree. Keeping it lazy is what makes gate-19 and the cold-start budget hold; do not add a static `import` of this group anywhere render-reachable.
