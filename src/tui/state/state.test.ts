...
import type { LineConfig, WidgetConfig } from "../../data/config/types.js";
...
describe("undo / redo", () => {
  it("undoes the last line-mutating action", () => {
    let s: EditorState = multiLine([["a", "b", "c"]]);
    expect(typesOf(s)[0]).toEqual(["a", "b", "c"]);
    s = reduce(s, { type: "delete" });
    expect(typesOf(s)[0]).toEqual(["b", "c"]);
  ...
  it("clears the redo future when a fresh edit branches off after an undo", () => {
    let s: EditorState = multiLine([["a", "b", "c"]]);
    s = reduce(s, { type: "delete" }); // remove 'a'
    s = reduce(s, { type: "delete" }); // remove 'b'
    s = reduce(s, { type: "undo" }); // back to ['b','c']
    expect(s.history.future).toHaveLength(1);
    s = reduce(s, { type: "delete" }); // fresh branch: remove 'b' again
    expect(s.history.future).toHaveLength(0);
    // The previously-redoable state is no longer reachable
    const before = typesOf(s)[0];
    s = reduce(s, { type: "redo" });
    expect(typesOf(s)[0]).toEqual(before);
  });

  it("ignores undo/redo when there is no history", () => {
    let s: EditorState = multiLine([["a"]]);
    const ref = s;
    s = reduce(s, { type: "undo" });
    expect(s).toBe(ref);
    s = reduce(s, { type: "redo" });
  ...
  it("ignores cursor moves — they do not consume an undo step", () => {
    let s: EditorState = multiLine([["a", "b", "c"]]);
    s = reduce(s, { type: "delete" });
    s = reduce(s, { type: "move-cursor", dx: 1 });
    s = reduce(s, { type: "undo" });
  ...
  it("undo/redo are no-ops in picker mode", () => {
    let s: EditorState = multiLine([["model"]]);
    s = reduce(s, { type: "delete" }); // record one history entry
    s = reduce(s, { type: "open-picker", intent: "add" });
    const ref = s;
    s = reduce(s, { type: "undo" });
    expect(s).toBe(ref);
  ...
  it("caps past history at HISTORY_CAP", () => {
    let s: EditorState = multiLine([Array.from({ length: 80 }, (_, i) => `w${i}`)]);
    for (let i = 0; i < 80; i += 1) {
      s = reduce(s, { type: "delete" });
    }
    expect(s.history.past.length).toBeLessThanOrEqual(HISTORY_CAP);
