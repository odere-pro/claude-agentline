 ...
 *
 * `bgRerender` chains the post-save background render passes (each
 * `useSaveFlow` invocation queues `triggerBackgroundRerender` behind the
 * previous one) so rapid consecutive saves can never land their
 * `last-render.json` writes out of order — without it, save N's
 * background render could finish *after* save N+1's and clobber the
 * fresher cache with stale output.
 */
export interface SaveTracker {
  inFlight: Promise<void> | null;
  bgRerender: Promise<void> | null;
}

export function createSaveTracker(): SaveTracker {
  return { inFlight: null, bgRerender: null };
}
 ...
 * `SIGNAL_EXIT_OFFSET + signal_number`. SIGTERM = 15.
 */
const SIGNAL_EXIT_OFFSET = 128;
const SIGTERM_SIGNAL = 15;
const SIGTERM_EXIT_CODE = SIGNAL_EXIT_OFFSET + SIGTERM_SIGNAL;
...
export function enterAltScreen(
  ...
  const onSignal = (signal: NodeJS.Signals): void => {
    ...
    if (inFlight) {
       ...
       * The save callback already surfaces errors to the editor's
       * status line; the leading `.catch` swallows any leaked
       * rejection so it can't fire as `unhandledRejection` *before*
       * the `.finally` runs `process.exit`.
       */
      void inFlight
        .catch(() => undefined)
        .finally(() => {
