...
import { DEFAULT_KEY_BINDINGS } from "../../keys/bindings/bindings.js";
...
describe("footerLines", () => {
  it("splits edit-scope bindings into motion (line 1) and actions (line 2)", () => {
    ...
    // `↵ add` is suppressed — only the `a` add verb is advertised.
    expect(actions).not.toContain("↵");
    expect(actions.startsWith("a add")).toBe(true);
  });
  ...
  it("picker-group footer surfaces the `/` search shortcut", () => {
    const { actions } = footerLines(DEFAULT_KEY_BINDINGS, "picker-group");
...
describe("enterAltScreen", () => {
  ...
  it("writes the alt-screen enter sequence + a clean-canvas reset on call, and the leave sequence on restore", () => {
    ...
    const restore = enterAltScreen(stream);
    expect(writes).toEqual([`${ESC}?1049h`, `${ESC}2J${ESC}3J${ESC}H`]);
    restore();
    expect(writes).toEqual([`${ESC}?1049h`, `${ESC}2J${ESC}3J${ESC}H`, `${ESC}?1049l`]);
  });
  ...
  it("restore is idempotent — calling twice does not double-write the leave sequence", () => {
    ...
    restore();
    restore();
    expect(writes.filter((s) => s === `${ESC}?1049l`)).toHaveLength(1);
  });
  ...
  it("a SIGINT during the session triggers the leave sequence", () => {
     ...
     */
    process.emit("SIGINT");
    expect(writes).toContain(`${ESC}?1049l`);
...
describe("fullscreenStream", () => {
  ...
  it("prepends erase-screen + erase-scrollback + cursor-home on writes that look like Ink frames", () => {
    ...
    expect(writes).toEqual([
      `${ESC}2J${ESC}3J${ESC}Hframe-one\n`,
      `${ESC}2J${ESC}3J${ESC}Hframe-two\n`,
    ]);
  });
  ...
  it("passes short ANSI control writes through without the FULLSCREEN_RESET prefix", () => {
    ...
    const { stream, writes } = makeStream(true);
    const wrapped = fullscreenStream(stream);
    wrapped.write(`${ESC}?25l`); // cliCursor.hide
    wrapped.write(`${ESC}?25h`); // cliCursor.show
    wrapped.write(`${ESC}2K${ESC}1A${ESC}G`); // log-update's eraseLines(1)
    expect(writes).toEqual([`${ESC}?25l`, `${ESC}?25h`, `${ESC}2K${ESC}1A${ESC}G`]);
  });
  ...
  it("handles Uint8Array chunks by converting to string before checking the frame suffix", () => {
    ...
    expect(writes).toEqual([`${ESC}2J${ESC}3J${ESC}Hbuffered\n`]);
  });
});
