...
import { familiesWithWidgets } from "../../picker/picker.js";
import type { SaveTracker } from "../mount.js";
import { initialState, reduce, type EditorAction, type EditorState } from "../../state/state.js";
...
describe("handleEditKey", () => {
  const makeEditDeps = (
    ...
    const saveTracker: SaveTracker = { inFlight: null, bgRerender: null };
    return {
      dispatch,
      exit,
      onSave,
  ...
  it("Ctrl-Z dispatches undo; Ctrl-Shift-Z and Ctrl-Y dispatch redo", () => {
    const { deps, dispatch } = makeEditDeps();
    handleEditKey("z", key({ ctrl: true }), deps);
    expect(dispatch).toHaveBeenCalledWith({ type: "undo" });
    handleEditKey("Z", key({ ctrl: true, shift: true }), deps);
    expect(dispatch).toHaveBeenCalledWith({ type: "redo" });
    handleEditKey("y", key({ ctrl: true }), deps);
    expect(dispatch).toHaveBeenLastCalledWith({ type: "redo" });
  });

  it("s is suppressed while a save is already in flight", () => {
    const tracker: SaveTracker = { inFlight: Promise.resolve(), bgRerender: null };
