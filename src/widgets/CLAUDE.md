...
- One subdirectory per **widget family** (the named groups defined in `docs/GLOSSARY.md` and `src/widgets/catalog/types.ts`).
- `catalog.ts` + `catalog/<family>.ts` — the static `WIDGET_CATALOG` metadata (human name, description, family, variants).
- `registry.ts` — the runtime `WidgetRegistry` mapping `type` → render function, populated by `registerAllBuiltins()`.
- `types.ts` — the widget contract (`Cell`, `WidgetContext`, `WidgetRender`, `WidgetDef`); `family-identity.ts` — the one map for a family's glyph + accent.
...
- Fully **render-reachable** — no `ink`/`react`/`src/tui/` imports (gate-19). `family-identity.ts` is deliberately type-only-plus-`unicodeCapable` so it can be shared with the editor preview without dragging the render graph into the cold path.
