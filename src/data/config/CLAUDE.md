...
- **An unrecognised `widget.type` is tolerated, never hard-failed.** `validate.ts` filters the schema `enum` error at `/lines/<n>/widgets/<m>/type` (issue #311). The `widget.type` enum is generated from `WIDGET_CATALOG` (gate-28), so without this a config naming a retired widget fails validation outright and the user loses **every** line, not just that cell — the break PR #206 shipped for its eight retired types. The unknown widget stays in the config, the render hides it silently, and `agentline doctor` D11 names it. Do not add a statusline warning row for it: that would reintroduce the variable physical-row count removed in #304. Authoring stays strict — `addWidget` / `replaceWidget` validate against the catalogue before writing.
...
- **Mutation ops reject unknown option keys / out-of-range values at authoring time.** `addWidget` / `replaceWidget` / `setWidgetOption` in `mutate.ts` validate `options` against the catalogue-derived `validateWidgetOption` / `validateWidgetOptions` (`src/widgets/families/option-spec/option-spec.ts`) and throw `ConfigMutationError` with a catalogue-pointing message (the CLI maps it to exit 2). The render path stays forgiving (a bad value falls back); the _authoring_ surface is strict. `label` is the universal option; `reset` is valid only on the token accumulators and is checked against `RESET_AXES`. Add a widget's validatable keys/values to the option spec, not a second validator.
