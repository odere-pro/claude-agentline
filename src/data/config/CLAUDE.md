...
- **The only write path is the atomic helper.** Every persisted config write goes through `writeJsonIdempotent` from `src/core/lib/atomic-write.ts` (write sibling temp → `fsync` → `rename`). Never `fs.writeFile` the user config directly anywhere in this leaf.
