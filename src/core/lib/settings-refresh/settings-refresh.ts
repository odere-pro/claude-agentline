...
import { writeJsonIdempotent } from "../atomic-write/atomic-write.js";
import { withFileLock } from "../file-lock/file-lock.js";
 ...
 *
 * The read → mutate → write cycle runs under `withFileLock` so a
 * concurrent `agentline install` (or another `syncRefreshInterval`
 * caller) cannot mutate settings.json between the read and the rename
 * and silently clobber the other writer's net changes.
 */
