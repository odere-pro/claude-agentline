...
- `i18n/` — config-driven user-facing string routing. `en-dictionary.ts` is the single source of truth for authored English text (every `app.*`, `picker.*`, `family.*`, `footer.*`, `cmd.*` id); `ids.ts` declares `I18N_NAMESPACES`; `loader.ts` exposes `createTranslator` (catalogue/template-literal form) and `createDictTranslator` (dictionary-bound form) resolved once per render tick.
