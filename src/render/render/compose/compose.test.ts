...
describe("pickComposeStrategy", () => {
  it("returns plainCompose when powerline is disabled", () => {
    expect(pickComposeStrategy(baseOptions())).toBe(plainCompose);
  });

  it("returns powerlineCompose when powerline is enabled", () => {
    expect(pickComposeStrategy(baseOptions({}, { enabled: true }))).toBe(powerlineCompose);
  });

  it("composeLines produces the same output as calling the picked strategy directly", () => {
    const lines = [[cell({ text: "a" }), cell({ text: "b" })]];
    const opts = baseOptions();
    expect(composeLines(lines, opts)).toEqual(pickComposeStrategy(opts)(lines, opts));
  });
});
