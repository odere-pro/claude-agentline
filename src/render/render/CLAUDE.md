...
- **No ink/react/TUI import — enforced by `gate-19`.** Every `.ts` under `src/` outside `src/tui/` is scanned for static imports of `ink`, `react`, or any `./tui/` / `../tui/` path. A single accidental top-level TUI import here blows the cold-start budget by multiples. The only legitimate TUI entry is the runtime URL import in `src/cli.ts` (a string, not a static import).
