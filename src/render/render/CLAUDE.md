...
- **Bidi / format controls are stripped at `sanitize.ts`.** The `CONTROL_CHARS` regex covers C0/DEL/C1 plus the Unicode bidi set (U+200E/200F, U+202A–U+202E, U+2028/2029, U+2066–U+2069). This is the single chokepoint so every widget benefits automatically; it runs before width measurement, so the measured width matches the bytes that reach the terminal.
- **Width math is grapheme-aware and never splits an escape.** Composition measures East-Asian wide, emoji, and ZWJ sequences via `width.ts`; truncation/ellipsis must land on a cell boundary and never inside an ANSI escape sequence. When no real width signal exists (`detected: false`), the pipeline composes against `NO_WRAP_WIDTH` with `noWrap: true` — one row per configured line, host clips horizontally — rather than wrapping a guessed 80. When a width **is** detected, compose against all of it — the renderer reserves no columns for host chrome, because anything held back silently hides a widget the user configured (issue #318).
...
- **No network at render time.** This leaf makes no outbound requests, ever (`gate-14`). The exception is the user-opted-in `git-pr` widget: when ANY `git-pr` widget in the resolved config has `options.allowNetwork === true`, `loadLiveSnapshots` in `context.ts` passes `allowPullRequest: true` to `loadGitSnapshot`. The default path (no `git-pr` + `allowNetwork`) is always network-free. The `hasAllowNetworkGitPr` helper in `context.ts` is the single gating point.
