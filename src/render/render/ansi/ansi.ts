...
import { parseColour, type Colour, type ParsedColour } from "../../../data/theme/colours/colours.js";
...
function sanitiseHref(href: string): string {
  /*
   * Strip C0 control characters (0x00–0x1f), DEL (0x7f), and C1 controls
   * (0x80–0x9f). C1 must go too — some terminals interpret `\x9b` as
   * CSI and would re-open an escape sequence inside the URL. Everything
   * else — spaces, brackets, percent-encoded bytes — is left alone; the
   * terminal does its own parsing on the URL.
   */
  // eslint-disable-next-line no-control-regex
  return href.replace(/[\x00-\x1f\x7f-\x9f]/g, "");
}
...
function rgbToAnsi256(rgb: Rgb): number {
  if (rgb.r === rgb.g && rgb.g === rgb.b) {
    ...
    // Upper bound of the ramp's input range == divisor (255 − ANSI_GREY_OFFSET).
    if (rgb.r > ANSI_GREY_RAMP_DIVISOR) return ANSI_GREY_TOP;
    return (
