...
import { HelpRequestedError, isHelpFlag, requestHelp } from "../core/lib/help/help.js";
...
import { runRedoSubgroup } from "../data/config/redo/redo-command.js";
...
function runHelp(): number {
  process.stdout.write(
    [
      ...
      "  config redo          re-apply a change rolled back by undo",
...
Groups:
  ...
  redo                             re-apply a change rolled back by undo
...
async function runConfig(rest: readonly string[]): Promise<number> {
  ...
  switch (group) {
    ...
    case "redo":
      return runRedoSubgroup(groupRest);
    default:
