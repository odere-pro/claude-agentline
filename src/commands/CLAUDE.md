...
- `cli/` — the help-string utilities (separate from the dispatch entry in `src/cli.ts`).
...
- Render-reachable in the sense that `src/cli.ts` imports the arg-parsers/dispatchers at the top — so this group must stay `ink`/`react`/`src/tui/`-free (gate-19). Only `agentline edit` pulls the TUI, via the lazy URL import.
...
- Allowed import direction: `commands` imports from `core`/`data`/`render`; it does not import `tui` (the editor verb dispatches through the lazy URL in `src/cli.ts`).
